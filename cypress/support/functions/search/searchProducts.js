...
const pesquisarProdutos = (produtoDesejado) => {
    cy.fixture('listProducts').then((dados) => {
      ...
      if (produtoDesejado || produtoDesejado.length >= 1) {
        cy.get(NAV_BAR.BARRA_DE_PESQUISA.CAMPOS_PESQUISA)
          .type(produtoDesejado);
        cy.get(NAV_BAR.BARRA_DE_PESQUISA.BOTÃO_PESQUISAR)
          .click();
      } else {
        throw new Error(`Teste falhou. Nenhum produto desejado. Favor revisar o código`);
      }
    });
  };
...
const validarPesquisaProduto = (especificacao1, especificacao2) => {
    ...
    cy.get(PAGINA_PRODUTO.TITULO_PRODUTO).each(($title, index, $titles) => {
      ...
      const isKeywordPresent = PALAVRAS_CHAVE.some((PALAVRA_CHAVE) => titleText.includes(PALAVRA_CHAVE.toLowerCase())); // Converte a palavra-chave para minúsculas
  
      if (isKeywordPresent && !elementoEncontrado) {
